...
(import ./run-assertions.nix { inherit pkgs; }).run "discover-user-registry-tests" {
  # a subdirectory with configuration.nix (no home.nix) becomes a "<name>@*"
  ...
  symlink-to-user-included =
    toString (result."link-to-dennis@*" or null) == entryPath "link-to-dennis";
