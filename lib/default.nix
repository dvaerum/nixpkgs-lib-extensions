...
let
  ...
  import_file =
    ...
    in
    if builtins.isFunction unknown_import then
      unknown_import self
    else if builtins.isAttrs unknown_import then
      unknown_import
    else
      throw "The file `${folder_name}/${file_name}` has to contain either an attribute set or a function";
...
lib.fix (
  ...
  let
    ...
    libraries = (
      lib.genAttrs dirNames (
        folder_name:
        if (builtins.pathExists ./${folder_name}/default.nix) then
          import ./${folder_name} { inherit lib; }
        else
          (lib.mergeAttrsList (lib.forEach (get_file_names ./${folder_name}) (import_file self folder_name)))
      )
    );
